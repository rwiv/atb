...
`app` 모듈은 `atb` 프로젝트의 실행 컨텍스트를 캡슐화하고 전체 비즈니스 로직을 오케스트레이션하는 최상위 레이어입니다.

## 모듈 구조

- **`mod.rs`**: 애플리케이션의 메인 실행기인 `App` 구조체를 정의하며 전체 흐름을 제어합니다.
- **`cli.rs`**: `clap`을 사용한 CLI 인자(`Cli`) 및 명령어(`Commands`) 규격을 정의합니다.
- **`config.rs`**: `toolkit.yaml` 파일을 파싱하고 경로 확장(`~`)을 담당하는 설정 모델입니다.
...
- **라이브러리 확장성**: 외부 프로젝트에서 `atb`를 라이브러리로 참조하여 고유한 워크플로우를 구축하기 용이합니다.
