...
fn test_e2e_dependency_missing_skill() {
    ...
    // researcher depends on web_search, but web_search is missing in toolkit.yaml
    let config = format!(
        r#"
source: {}
target: gemini-cli
...
"#,
    ...
    cmd.assert()
        ...
        .stderr(predicates::str::contains(
            "agent 'plugin_a:researcher' requires skill 'plugin_b:web_search' but it is missing in toolkit.yaml",
        ));
}
...
fn test_e2e_dependency_circular() {
    ...
    let p1_cmds = root.join("p1/commands");
    let p2_cmds = root.join("p2/commands");
    ...
    fs::write(
        root.join("p1/requirements.yaml"),
        "commands:
  c1:
    commands:
      - p2:c2",
    ...
    .unwrap();
    fs::write(
        root.join("p2/requirements.yaml"),
        "commands:
  c2:
    commands:
      - p1:c1",
...
fn test_e2e_dependency_complex() {
    ...
    let p1_cmds = root.join("p1/commands");
    let p2_agents = root.join("p2/agents");
    let p3_skills = root.join("p3/skills/s1");
...
agents:
  a1:
    ...
    skills:
      - p3:s1
"#;
    fs::write(root.join("p2/requirements.yaml"), deps_yaml).unwrap();
...
fn setup_dependency_fixtures(root: &Path) {
    let plugin_a_agents = root.join("plugin_a/agents");
    let plugin_b_skills = root.join("plugin_b/skills/web_search");
    ...
    fs::write(
        root.join("plugin_a/requirements.yaml"),
        r#"
agents:
  researcher:
    skills:
