...
본 문서는 `agb` 시스템을 구성하는 기술 스택, 데이터 처리 규칙, 타겟별 변환 및 동기화 명세, 그리고 운영 제약 사항을 정의합니다. 파일 구조 및 설정 파일 규격에 대해서는 [format.md](./format.md)를 참조하십시오.

## 1. 기술 스택 (Tech Stack)

`agb`는 다음 기술들을 기반으로 구축되었습니다:

- **언어**: Rust
- **CLI 프레임워크**: `clap`
- **데이터 직렬화**: `serde`, `serde_yaml`, `serde_json`, `toml`
...
2.  **Metadata Map**: `map.yaml`에 정의된 타겟별 필드 값 치환 규칙.
...
- 설정 파일(`agb.yaml`) 미존재 시 가이드 메시지와 함께 실행을 중단합니다.
