...
이 문서는 `atb` 시스템에서 사용하는 모든 파일의 물리적 구조, 설정 규격(`toolkit.yaml`), 그리고 리소스 작성 형식을 정의합니다.
...
`toolkit.yaml`이 위치한 디렉터리가 빌드 루트가 되며, 결과물은 해당 위치에 다음과 같은 구조로 생성됩니다.
...
### 4.3 의존성 정의 (`requirements.yaml`)

플러그인 루트에서 리소스 간의 의존성을 명시적으로 정의합니다. 빌드 시 `toolkit.yaml`에 포함되지 않은 리소스를 의존하고 있을 경우 빌드가 실패합니다.
